...
function Create( xMin, xMax, yMin, yMax, seed, data )
	...
	local jWorld = sm.json.open( "$CONTENT_DATA/Terrain/Worlds/WonkShipWorld_02.world") --WonkShip  ---!!DEV CHANGE
...
function GetCreationsForCell( cellX, cellY, lod )
	local creations = sm.terrainTile.getCreationsForCell( GetTileLoadParamsFromCellPos( cellX, cellY, lod ) )
...
function GetPrefabsForCell( cellX, cellY, lod )
	local prefabs = sm.terrainTile.getPrefabsForCell( GetTileLoadParamsFromCellPos( cellX, cellY, lod ) )
	for _, prefab in ipairs( prefabs ) do
		local rx, ry = RotateLocal( cellX, cellY, prefab.pos.x, prefab.pos.y )
		prefab.pos = sm.vec3.new( rx, ry, prefab.pos.z )
		prefab.rot = GetRotationQuat( cellX, cellY ) * prefab.rot
	end
	return prefabs
end
